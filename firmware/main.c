/* Name: main.c
 * Project: LCD2USB; lcd display interface based on AVR USB driver
 * Author: Till Harbaum
 * Creation Date: 2006-01-20
 * Tabsize: 4
 * Copyright: (c) 2005 by Till Harbaum <till@harbaum.org>
 * License: GPL
 * This Revision: $Id: main.c,v 1.2 2007/01/14 12:12:27 harbaum Exp $
 */

#include <avr/io.h>
#include <avr/interrupt.h>
#include <avr/pgmspace.h>
#include <avr/wdt.h>
#include <avr/eeprom.h>

#include <util/delay.h>

#include "lcd.h"

// use avrusb library
#include "usbdrv.h"
#include "oddebug.h"

#define VERSION_MAJOR 1
#define VERSION_MINOR 10
#define VERSION_STR "1.10"
// change USB_CFG_DEVICE_VERSION in usbconfig.h as well

#define LED_PORT C
#define LED_PIN 2




#define DDR(x) (*(&x - 1))      /* address of data direction register of port x */ 

/* bitmask of detected lcd controllers */
/////uchar controller = 0;

// EEMEM wird bei aktuellen Versionen der avr-lib in eeprom.h definiert
// hier: definiere falls noch nicht bekannt ("alte" avr-libc)
#ifndef EEMEM
// alle Textstellen EEMEM im Quellcode durch __attribute__ ... ersetzen
#define EEMEM  __attribute__ ((section (".eeprom")))
#endif

/* ------------------------------------------------------------------------- */
/* PWM units are used for contrast and backlight brightness */

/* contrast and brightness are stored in eeprom */
uchar eeprom_valid EEMEM;
uchar eeprom_contrast EEMEM;
uchar eeprom_brightness EEMEM;
// флаг запуска bootloader'а
uchar eeprom_bootload EEMEM;


// таблица перекодировки русских символов
uchar translation_table[] = {
        ,
        162,  // Ё
        0xa9,
        0xaa,
        200,  // «
        201,  // »
        0xad,
        0xae,
        0xaf,
        0xb0,
        0xb1,
        0xb2,
        0xb3,
        0xb4,
        0xb5,
        0xb6,
        0xb7,
        181,  // ё
        0xb9,
        0xba,
        0xbb,
        0xbc,
        0xbd,
        0xbe,
        0xbf,
        0x41, // А
        160,  // Б
        0x42, // В
        161,  // Г
        224,  // Д
        0x45, // Е
        163,  // Ж
        164,  // З
        165,  // И
        166,  // Й
        0x4b, // К
        167,  // Л
        0x4d, // М
        0x48, // Н
        0x4f, // О
        168,  // П
        0x50, // Р
        0x43, // С
        0x54, // Т
        169,  // У
        170,  // Ф
        0x58, // Х
        225,  // Ц
        171,  // Ч
        172,  // Ш
        226,  // Щ
        173,  // Ъ
        174,  // Ы
        0x62, // Ь
        175,  // Э
        176,  // Ю
        177,  // Я
        0x61, // а
        178,  // б
        179,  // в
        180,  // г
        227,  // д
        0x65, // е
        182,  // ж
        183,  // з
        184,  // и
        185,  // й
        186,  // к
        187,  // л
        188,  // м
        189,  // н
        0x6f, // о
        190,  // п
        0x70, // р
        0x63, // с
        191,  // т
        0x79, // у
        228,  // ф
        0x78, // х
        229,  // ц
        192,  // ч
        193,  // ш
        230,  // щ
        194,  // ъ
        195,  // ы
        196,  // ь
        197,  // э
        198,  // ю
        199   // я
};

void inline led_init() {
	USB_DDRPORT(LED_PORT) |= _BV(LED_PIN);
}

void inline led_on() {
	USB_OUTPORT(LED_PORT) &= ~(_BV(LED_PIN));
}

void inline led_off() {
	USB_OUTPORT(LED_PORT) |= _BV(LED_PIN);
}

/* 
// функция мигания светодиодом
// нужна для отладки
static void inline debug_flash() {
	int i;
	led_init();
	for (i=0; i<6; i++) {
		if (USB_OUTPORT(LED_PORT) & _BV(LED_PIN)) {
			USB_OUTPORT(LED_PORT) &= ~(_BV(LED_PIN));
		} else {
			USB_OUTPORT(LED_PORT) |= _BV(LED_PIN);
		}
		_delay_ms(50);
		wdt_reset();
	}
	led_off();
} */

static void initForUsbConnectivity(void)
{
	uchar   i = 0;

#if F_CPU == 12800000
    TCCR0 = 3;          /* 1/64 prescaler */
#endif
    usbInit();
	cli();
    /* enforce USB re-enumerate: */
    usbDeviceDisconnect();  /* do this while interrupts are disabled */
    do{             /* fake USB disconnect for > 250 ms */
        wdt_reset();
        _delay_ms(1);
    }while(--i);
    usbDeviceConnect();
    sei();
}

void pwm_init(void) {

  /* check if eeprom is valid and set default values if not */
  /* initial values: full contrast and full brightness */
  if(eeprom_read_byte(&eeprom_valid) != 0x42) {

    /* write magic "eeprom is valid" marker 0x42 and default values */
    eeprom_write_byte(&eeprom_valid, 0x42);
    eeprom_write_byte(&eeprom_contrast, 0xff);
    eeprom_write_byte(&eeprom_brightness, 0xff);
  }

  /* PortB: set DDB1 and DDB2 => PORTB1 and PORTB2 are output */
  DDRB |= _BV(1) | _BV(2);
 
  /* Set Timer1:
    - Fast PWM,8bit => Mode 5 (WGM13=0,WGM12=1,WGM11=0,WGM10=1)
    - Output-Mode: lower voltage is higher contrast
      => Set OC1A on Compare Match, clear OC1A at BOTTOM, (inverting mode)
	 COM1A1=1,COM1A0=1
	 higher voltage is higher brightness
      => Clear OC1B on Compare Match, set OC1B at BOTTOM, (non-inverting mode)
	 COM1B1=1,COM1B0=0
    - Timer runs at inernal clock with no prescaling => CS12=0,CS11=0,CS10=1
  */
  TCCR1A = _BV(COM1A1) | _BV(COM1A0) | _BV(COM1B1) | _BV(WGM10);
  TCCR1B = _BV(WGM12) | _BV(CS10);

  TIMSK &=( (~_BV(2)) & (~_BV(3)) & (~_BV(4)) & (~_BV(5)));

  OCR1A = eeprom_read_byte(&eeprom_contrast);
  OCR1B = eeprom_read_byte(&eeprom_brightness);
}

void set_contrast(uchar value) {
  /* store value in eeprom if it actually changed */
  if(value != eeprom_read_byte(&eeprom_contrast))
    eeprom_write_byte(&eeprom_contrast, value);

  OCR1A = value;  // lower voltage is higher contrast
}

void set_brightness(uchar value) {
  /* store value in eeprom if it actually changed */
  if(value != eeprom_read_byte(&eeprom_brightness))
    eeprom_write_byte(&eeprom_brightness, value);

  OCR1B = value;  // higher voltage is higher brightness
}

/* ------------------------------------------------------------------------- */

uchar usbFunctionSetup(uchar data[8]) {
  static uchar replyBuf[4];
  usbMsgPtr = replyBuf;
  uchar len = (data[1] & 3)+1;       // 1 .. 4 bytes 
  uchar target = (data[1] >> 3) & 3; // target 0 .. 3
  uchar i;

  // request byte:

  // 7 6 5 4 3 2 1 0
  // C C C T T R L L

  // TT = target bit map 
  // R = reserved for future use, set to 0
  // LL = number of bytes in transfer - 1 

  switch(data[1] >> 5) {

  case 0: // echo (for transfer reliability testing)
    replyBuf[0] = data[2];
    replyBuf[1] = data[3];
    return 2;
    break;
    
  case 1: // command
      for(i=0;i<len;i++)
		lcd_command(data[2+i]);
    break;

  case 2: // data
    for(i=0;i<len;i++)
		// Возможно, следующая запись будет глючить
		// возможно, эти данные являются не только символами, но и параметрами команд...
		// короче, нужно еще тестировать в реале...
		lcd_data(translation_table[data[2+i]]); 
		//lcd_data(data[2+i]);
    break;

  case 3: // set
    switch(target) {

    case 0:  // contrast
      set_contrast(data[2]);
      break;

    case 1:  // brightness
      set_brightness(data[2]);
      break;

    default:
      // must not happen ...
      break;      
    }
    break;

  case 4: // get
    switch(target) {
    case 0: // version
      replyBuf[0] = VERSION_MAJOR;
      replyBuf[1] = VERSION_MINOR;
      return 2;
      break;

    case 1: // keys
      replyBuf[0] = ((PINB & _BV(0))?0:1) | 
	            ((PINB & _BV(3))?0:2);
      replyBuf[1] = 0;
      return 2;
      break;

    case 2: // controller map
      replyBuf[0] = 1<<0;
      replyBuf[1] = 0;
      return 2;
      break;      

    default:
      // must not happen ...
      break;      
    }
    break;
	case 5: // initialize bootloader
		eeprom_write_byte(&eeprom_bootload, 0x0); // пишем в ячейку EEPROM нулевое число
		_delay_ms(50);		// ждём завершения операции записи
		lcd_puts("bootloader\nstarting soon...");
		_delay_ms(5000); 
		cli();
		asm("RJMP 0x1800"); // скачем на нащ бутлодырь
	break;
  default:
    // must not happen ...
    break;
  }

  return 0;  // reply len
}

/* ------------------------------------------------------------------------- */
/*void translate_init() {
	uchar i;
	for (i = 0; i <=255; i++) {
		translation_table[i] = i;
		wdt_reset();
	}
	translation_table[0xc0] = 0x41; // A
	translation_table[0xc1] = 160;  // Б
	translation_table[0xc2] = 0x42; // В
	translation_table[0xc3] = 161;  // Г
	translation_table[0xc4] = 224;  // Д
	translation_table[0xc5] = 0x45; // Е
	translation_table[0xa8] = 162;  // Ё
	translation_table[0xc6] = 163;  // Ж
	translation_table[0xc7] = 164;  // З
	translation_table[0xc8] = 165;  // И
	translation_table[0xc9] = 166;  // Й
	translation_table[0xca] = 0x4b; // К
	translation_table[0xcb] = 167;  // Л
	translation_table[0xcc] = 0x4d; // М
	translation_table[0xcd] = 0x48; // Н
	translation_table[0xce] = 0x4f; // О
	translation_table[0xcf] = 168;  // П
	translation_table[0xd0] = 0x50; // Р
	translation_table[0xd1] = 0x43; // С
	translation_table[0xd2] = 0x54; // Т
	translation_table[0xd3] = 169;  // У
	translation_table[0xd4] = 170;  // Ф
	translation_table[0xd5] = 0x58; // Х
	translation_table[0xd6] = 225;  // Ц
	translation_table[0xd7] = 171;  // Ч
	translation_table[0xd8] = 172;  // Ш
	translation_table[0xd9] = 226;  // Щ
	translation_table[0xda] = 173;  // Ъ
	translation_table[0xdb] = 174;  // Ы
	translation_table[0xdc] = 0x62; // Ь
	translation_table[0xdd] = 175;  // Э
	translation_table[0xde] = 176;  // Ю
	translation_table[0xdf] = 177;  // Я
	wdt_reset();
	translation_table[0xe0] = 0x61; // а
	translation_table[0xe1] = 178;  // б
	translation_table[0xe2] = 179;  // в
	translation_table[0xe3] = 180;  // г
	translation_table[0xe4] = 227;  // д
	translation_table[0xe5] = 0x65; // е
	translation_table[0xb8] = 181;  // ё
	translation_table[0xe6] = 182;  // ж
	translation_table[0xe7] = 183;  // з
	translation_table[0xe8] = 184;  // и
	translation_table[0xe9] = 185;  // й
	translation_table[0xea] = 186;  // к
	translation_table[0xeb] = 187;  // л
	translation_table[0xec] = 188;  // м
	translation_table[0xed] = 189;  // н
	translation_table[0xee] = 0x6f; // о
	translation_table[0xef] = 190;  // п
	translation_table[0xf0] = 0x70; // р
	translation_table[0xf1] = 0x63; // с
	translation_table[0xf2] = 191;  // т
	translation_table[0xf3] = 0x79; // у
	translation_table[0xf4] = 228;  // ф
	translation_table[0xf5] = 0x78; // х
	translation_table[0xf6] = 229;  // ц
	translation_table[0xf7] = 192;  // ч
	translation_table[0xf8] = 193;  // ш
	translation_table[0xf9] = 230;  // щ
	translation_table[0xfa] = 194;  // ъ
	translation_table[0xfb] = 195;  // ы
	translation_table[0xfc] = 196;  // ь
	translation_table[0xfd] = 197;  // э
	translation_table[0xfe] = 198;  // ю
	translation_table[0xff] = 199;  // я
	wdt_reset();
	translation_table[0xab] = 200;  // «
	translation_table[0xbb] = 201;  // »
}*/

PROGMEM const char usbHidReportDescriptor[USB_CFG_HID_REPORT_DESCRIPTOR_LENGTH] = {    /* USB report descriptor */
    0x06, 0x00, 0xff,              // USAGE_PAGE (Generic Desktop)
    0x09, 0x01,                    // USAGE (Vendor Usage 1)
    0xa1, 0x01,                    // COLLECTION (Application)
    0x15, 0x00,                    //   LOGICAL_MINIMUM (0)
    0x26, 0xff, 0x00,              //   LOGICAL_MAXIMUM (255)
    0x75, 0x08,                    //   REPORT_SIZE (8)
    0x95, 0x08,                    //   REPORT_COUNT (8)
    0x09, 0x00,                    //   USAGE (Undefined)  
    0x82, 0x02, 0x01,              //   INPUT (Data,Var,Abs,Buf)
    0x95, HIDSERIAL_INBUFFER_SIZE, //   REPORT_COUNT (32)
    0x09, 0x00,                    //   USAGE (Undefined)        
    0xb2, 0x02, 0x01,              //   FEATURE (Data,Var,Abs,Buf)
    0xc0                           // END_COLLECTION
};

void lcd_putc_translated(uchar c) {
	lcd_putc(translation_table[c]);
}

void lcd_puts_translated(const uchar *s)
{
    register char c;

    while ( (c = *s++) ) {
        lcd_putc_translated(c);
    }

}/* lcd_puts_translated */



/*uchar translate_char(uchar c) {
	return translation_table[c];
}*/



/* ------------------------------------------------------------------------- */

int __attribute__((noreturn)) main(void)
{
	char buf[16];
	wdt_enable(WDTO_1S);
//	debug_flash();

    PORTB = _BV(0) | _BV(3); /* activate pull-up for key */ /*кнопки на пинах PB0 и PB3 */
    odDebugInit();
    DBG1(0x00, 0, 0);
	wdt_reset();
	pwm_init();
	wdt_reset();
    initForUsbConnectivity();
	//translate_init();
	//usbInit();
	lcd_init(LCD_DISP_ON);
	set_contrast(0xbf);
	set_brightness(0x7f);
	wdt_reset();
	lcd_puts_translated("LCD2USB " VERSION_STR " от\nЛукашевича Тимофея");
	//lcd_puts_translated("1");
	//sprintf(buf, "- %d -", sizeof(uchar));
/*	for(c = 0; c < 16; c++) {
		lcd_putc(c + 224);
	}
	lcd_putc('\n');
	for(c = 0; c < 16; c++) {
		lcd_putc(c + 224 + 16);
	}*/
	sei();
  
	for(;;) {	/* main event loop */
		wdt_reset();
		usbPoll();
	}
}



/* ------------------------------------------------------------------------- */
